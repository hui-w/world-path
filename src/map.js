...
*/
  ...
  renderPois: function (g) {
    var that = this;
    window.mapdata.pois.forEach(function (item) {
      ...
      //}
      if (
        item[Consts.KeyIndex] == "ORD" ||
        item[Consts.KeyIndex] == "LAX" ||
        item[Consts.KeyIndex] == "SFO" ||
        item[Consts.KeyIndex] == "YVR" ||
        item[Consts.KeyIndex] == "JFK"
      ) {
        that.renderPoi(g, item, that.width);
      }
  ...
  renderFlightsAndRoutes: function (g) {
    var that = this;
    window.mapdata.flights.forEach(function (flightGroup) {
      //each route group
      ...
      //next color
      window.mapdata.colorIndex++;
    });

    window.mapdata.routes.forEach(function (drivingGroup) {
      //each route group
  ...
  zoom: function (scaleRatio, point) {
    if ((this.scale < Config.WorldMap.minScale && scaleRatio < 1) || (this.scale > Config.WorldMap.maxScale && scaleRatio > 1)) {
           ...
           if (animation) {
               ...
               var moveStep = function() {
                   ...
                   if (that.left != x || that.top != y) {
                       setTimeout(moveStep, Config.WorldMap.moveTimerDelay);
                   } else {
  ...
  getPoi: function (key) {
    for (var i = 0; i < window.mapdata.pois.length; i++) {
      if (window.mapdata.pois[i][Consts.KeyIndex] == key) {
        return window.mapdata.pois[i];
      }
    }
    return null;
  },
