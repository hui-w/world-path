...
Frame.prototype = {
    moveMapTo: function(targetLeft, targetTop, callback) {
        ...
        var stepPosition = function() {
            //motion done
            if (that.map.left == targetLeft &&
                that.map.top == targetTop) {
                that.inMotion = false;
            ...
            //next round
            setTimeout(stepPosition, Config.Map.moveTimerDelay);
        };
        stepPosition();
    },
    ...
    scaleMapTo: function(targetScale, callback) {
        ...
        if (targetScale < Config.Map.minScale) {
            targetScale = Config.Map.minScale;
        } else if (targetScale > Config.Map.maxScale) {
            targetScale = Config.Map.maxScale;
        }
        ...
        var stepScale = function() {
            ...
            if ((ratio < 1 && nextScale < targetScale) ||
                (ratio > 1 && nextScale > targetScale)) {
            ...
            //next round
            setTimeout(stepScale, Config.Map.zoomTimerDelay);
        };
        stepScale();
    },
    ...
    renderInto: function(parent, renderComplete) {
        ...
        var cssStyle = Util.format_str("width: {width}px; height: {height}px; cursor: pointer; overflow: hidden; border: none; background-color: RGBA(100, 100, 100, 0.2);", {
            "width": this.width,
            "height": this.height
        });
    ...
    updateStatusLabel: function() {
        this.statusLabel.textContent = "[{0}, {1}] x {2}".format(this.map.left.toFixed(1), this.map.top.toFixed(1),
            this.map.scale.toFixed(1)
        );
    ...
    home: function() {
        ...
        this.setCenterAndScale(position.x, position.y, Config.Map.maxScale, function() {
            console.log("DONE");
        })
    },
